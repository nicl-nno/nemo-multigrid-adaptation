...
CONTAINS
   ...
   SUBROUTINE lim_wri( kindic )
	  ...
	  REAL(wp), DIMENSION(jpi,jpj) ::   u_ice_rotated, v_ice_rotated
      !!-------------------------------------------------------------------
      ...
      IF ( iom_use( "uice_ipa" ) .OR. iom_use( "vice_ipa" ) .OR. iom_use( "icevel" ) ) THEN 
		...
		CALL rot_rep(u_ice(:,:),v_ice(:,:), 'U', 'ij->e', prot_u)
		CALL rot_rep(u_ice(:,:),v_ice(:,:), 'V', 'ij->n', prot_v)
		u_ice_rotated(:,:) = prot_u
		v_ice_rotated(:,:) = prot_v
!

		!CALL rot_rep(z2da(:,:),z2db(:,:), 'U', 'ij->e', prot_u)
		!CALL rot_rep(z2da(:,:),z2db(:,:), 'V', 'ij->n', prot_v)
		!u_ice_rotated(:,:) = prot_u
		!v_ice_rotated(:,:) = prot_v
		
		CALL iom_put( "uice_ipa"     , u_ice_rotated             )       ! ice velocity u component
         CALL iom_put( "vice_ipa"     , v_ice_rotated             )       ! ice velocity v component
