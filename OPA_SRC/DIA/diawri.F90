...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: diawri.F90 7494 2016-12-14 09:02:43Z timgraham $
...
# if defined key_iomput
   ...
   SUBROUTINE dia_wri( kt )
	  ...
	  REAL(wp), DIMENSION(jpi,jpj,jpk) ::   un_rotated, vn_rotated    
      !!----------------------------------------------------------------------
      ! 
      ...
      DO jk = 1, jpk
					CALL rot_rep(un(:,:,jk),vn(:,:,jk), 'U', 'ij->e', prot_u)
					CALL rot_rep(un(:,:,jk),vn(:,:,jk), 'V', 'ij->n', prot_v)
					un_rotated(:,:,jk) = prot_u
					vn_rotated(:,:,jk) = prot_v
      ENDDO
      
      CALL iom_put( "uoce", un_rotated(:,:,:)         )    ! 3D i-current
      ...
      CALL iom_put( "voce", vn_rotated(:,:,:)         )    ! 3D j-current
