MODULE diaharm 
   ...
   REAL(wp), ALLOCATABLE, DIMENSION(:,:,:)   ::   out_u_rotated, out_v_rotated
...
902   IF( ios /= 0 ) CALL ctl_nam ( ios , 'nam_diaharm in configuration namelist', lwp )
   ...
   SUBROUTINE dia_harm_end
      ...
      ALLOCATE( out_eta(jpi,jpj,2*nb_ana),   & 
        ...
        &      out_u_rotated  (jpi,jpj,2*nb_ana),  &
		 &      out_v_rotated  (jpi,jpj,2*nb_ana))
   ...
   SUBROUTINE dia_wri_harm
      ...
      REAL(wp), DIMENSION(jpi,jpj) ::   prot_u, prot_v
      INTEGER  ::   jh, ji, jj
      !!----------------------------------------------------------------------
...
#else
      DO jh = 1, nb_ana
		CALL rot_rep(out_u(:,:,jh),out_v(:,:,jh       ), 'U', 'ij->e', prot_u)
		out_u_rotated(:,:,jh       ) = prot_u
        CALL iom_put( TRIM(tname(jh))//'x_u', out_u_rotated(:,:,jh) )
		
		CALL rot_rep(out_u(:,:,nb_ana+jh),out_v(:,:,nb_ana+jh), 'U', 'ij->e', prot_u)
		out_u_rotated(:,:,jh+nb_ana       ) = prot_u
		CALL iom_put( TRIM(tname(jh))//'y_u', out_u_rotated(:,:,nb_ana+jh) )
      END DO
#endif
...
#else
      DO jh = 1, nb_ana
         CALL rot_rep(out_u(:,:,jh),out_v(:,:,jh       ), 'V', 'ij->n', prot_v)
		 out_v_rotated(:,:,jh) = prot_v
         CALL iom_put( TRIM(tname(jh))//'x_v', out_v_rotated(:,:,jh       ) )
		 
		 CALL rot_rep(out_u(:,:,jh+nb_ana),out_v(:,:,jh+nb_ana       ), 'V', 'ij->n', prot_v)
		 out_v_rotated(:,:,jh+nb_ana       ) = prot_v
		 CALL iom_put( TRIM(tname(jh))//'y_v', out_v_rotated(:,:,jh+nb_ana) )
      END DO
#endif
      !
