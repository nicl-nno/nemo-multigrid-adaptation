MODULE geo2ocean
   ...
   PUBLIC   rot_rep, rot_rep_bdy, rot_rep_isolated, repcmo, repere, geo2oce, oce2geo   ! only rot_rep should be used
...
#  include "vectopt_loop_substitute.h90"
   ...
   !! $Id: geo2ocean.F90 4162 2013-11-07 10:19:49Z cetlod $ 
...
CONTAINS
   ...
   SUBROUTINE rot_rep_bdy ( pxin, pyin, indi, indj, cd_type, cdtodo, prot )
   REAL(wp), INTENT( IN ) ::   pxin, pyin   ! vector componantes
      ...
      REAL(wp), INTENT(out) ::   prot      

		INTEGER, INTENT( IN ) ::   indi, indj   ! vector start indices
	
		IF( lmust_init ) THEN
         IF(lwp) WRITE(numout,*)
         IF(lwp) WRITE(numout,*) ' rot_rep_bdy : geographic <--> stretche2d'
      ...
      CASE ('en->i')      ! 'en->i' est-north componantes to model i componante
         SELECT CASE (cd_type)
         CASE ('T')   ;   prot = pxin * gcost(indi,indj) + pyin * gsint(indi,indj)
         CASE ('U')   ;   prot = pxin * gcosu(indi,indj) + pyin * gsinu(indi,indj)
         CASE ('V')   ;   prot = pxin * gcosv(indi,indj) + pyin * gsinv(indi,indj)
         CASE ('F')   ;   prot = pxin * gcosf(indi,indj) + pyin * gsinf(indi,indj)
      ...
      CASE ('en->j')      ! 'en->j' est-north componantes to model j componante
         SELECT CASE (cd_type)
         CASE ('T')   ;   prot = pyin * gcost(indi,indj) - pxin * gsint(indi,indj)
         CASE ('U')   ;   prot = pyin * gcosu(indi,indj) - pxin * gsinu(indi,indj)
         CASE ('V')   ;   prot = pyin * gcosv(indi,indj) - pxin * gsinv(indi,indj)
         CASE ('F')   ;   prot = pyin * gcosf(indi,indj) - pxin * gsinf(indi,indj)
         CASE DEFAULT   ;   CALL ctl_stop( 'Only T, U, V and F grid points are coded' )
         END SELECT
...
END SUBROUTINE rot_rep_bdy	  


SUBROUTINE rot_rep_isolated ( pxin, pyin, indi, indj, cd_type, cdtodo, prot )
   REAL(wp), INTENT( IN ) ::   pxin, pyin   ! vector componantes
      CHARACTER(len=1),             INTENT( IN ) ::   cd_type      ! define the nature of pt2d array grid-points
      CHARACTER(len=5),             INTENT( IN ) ::   cdtodo       ! specify the work to do:
      !!                                                           ! 'en->i' east-north componantes to model i componante
      ...
      !!                                                           ! 'ij->e' model i-j componantes to east componante
      !!                                                           ! 'ij->n' model i-j componantes to east componante
	  REAL(wp) ::   gsinup, gcosup, gsinvp, gcosvp
      REAL(wp), INTENT(out) ::   prot      

		INTEGER, INTENT( IN ) ::   indi, indj   ! vector start indices
		!WRITE(numout,*) 'TIDAL ROTATION',indi,indj,cdtodo,cd_type
		!IF( lmust_init ) THEN
         !IF(lwp) WRITE(numout,*)
         !IF(lwp) WRITE(numout,*) ' rot_rep_isolated : geographic <--> stretche2d'
         !IF(lwp) WRITE(numout,*) ' ~~~~~    coordinate transformation'
         !
         !CALL angle_nompi       ! initialization of the transformation
		 
		 CALL angle_point(indi, indj, gsinup, gcosup, gsinvp, gcosvp)
         !lmust_init = .FALSE.
		!ENDIF 
		
		 !WRITE(numout,*) 'TIDAL INITIALIZED',indi,indj,cdtodo,cd_type
	  SELECT CASE (cdtodo)
      CASE ('en->i')      ! 'en->i' est-north componantes to model i componante
         SELECT CASE (cd_type)
         !CASE ('T')   ;   prot = pxin * gcost(indi,indj) + pyin * gsint(indi,indj)
         CASE ('U')   ;   prot = pxin * gcosup + pyin * gsinup
         CASE ('V')   ;   prot = pxin * gcosvp + pyin * gsinvp
         !CASE ('F')   ;   prot = pxin * gcosf(indi,indj) + pyin * gsinf(indi,indj)
         CASE DEFAULT   ;   CALL ctl_stop( 'Only T, U, V and F grid points are coded' )
         END SELECT
      CASE ('en->j')      ! 'en->j' est-north componantes to model j componante
         SELECT CASE (cd_type)
         !CASE ('T')   ;   prot = pyin * gcost(indi,indj) - pxin * gsint(indi,indj)
         CASE ('U')   ;   prot = pyin * gcosup - pxin * gsinup
         CASE ('V')   ;   prot = pyin * gcosvp - pxin * gsinvp
         !CASE ('F')   ;   prot = pyin * gcosf(indi,indj) - pxin * gsinf(indi,indj)
         CASE DEFAULT   ;   CALL ctl_stop( 'Only T, U, V and F grid points are coded' )
         END SELECT
      CASE ('ij->e')      ! 'ij->e' model i-j componantes to est componante
	  ...
	  !WRITE(numout,*) 'TIDAL ROTATED',indi,indj

		
		
END SUBROUTINE rot_rep_isolated	  
   
SUBROUTINE angle_point ( iind, jind, gsinup, gcosup, gsinvp, gcosvp )
 ...
 !!----------------------------------------------------------------------
      ...
      !!----------------------------------------------------------------------
      INTEGER, INTENT( IN ) ::   iind, jind   ! dummy loop indices
      ...
      !!----------------------------------------------------------------------
	  REAL(wp), INTENT(out) ::   gsinup, gcosup, gsinvp, gcosvp

		!WRITE(numout,*) "ANGLE-TEST1",jpiglo,jpjglo
!ALLOCATE( gsint(jpiglo,jpjglo), gcost(jpiglo,jpjglo),   & 
!         &      gsinu(jpiglo,jpjglo), gcosu(jpiglo,jpjglo),   & 
!         &      gsinv(jpiglo,jpjglo), gcosv(jpiglo,jpjglo),   &  
!         &      gsinf(jpiglo,jpjglo), gcosf(jpiglo,jpjglo), STAT=ierr )
		! WRITE(numout,*) "ANGLE-TEST1.5"
      !IF(lk_mpp)   CALL mpp_sum( ierr )
	   !WRITE(numout,*) "ANGLE-TEST 1.75"
...
!      DO jj = 2, jpjglo
!CDIR NOVERRCHK
 !        DO ji = fs_2, jpiglo   ! vector opt.

            ! north pole direction & modulous (at t-point)
            zlam = glamt(iind,jind)
            zphi = gphit(iind,jind)
            ...
            zlam = glamu(iind,jind)
            zphi = gphiu(iind,jind)
            ...
            zlam = glamv(iind,jind)
            zphi = gphiv(iind,jind)
            ...
            zlam = glamf(iind,jind)
            zphi = gphif(iind,jind)
            ...
            zlam = glamv(iind,jind  )
            zphi = gphiv(iind,jind  )
            zlan = glamv(iind,jind-1)
            zphh = gphiv(iind,jind-1)
            ...
            zlam = glamf(iind,jind  )
            zphi = gphif(iind,jind  )
            zlan = glamf(iind,jind-1)
            zphh = gphif(iind,jind-1)
            ...
            zlam = glamf(iind  ,jind)
            zphi = gphif(iind  ,jind)
            zlan = glamf(iind-1,jind)
            zphh = gphif(iind-1,jind)
            ...
            zlam = glamu(iind,jind+1)
            zphi = gphiu(iind,jind+1)
            zlan = glamu(iind,jind  )
            zphh = gphiu(iind,jind  )
            ...
            !gsint(iind,jind) = ( zxnpt*zyvvt - zynpt*zxvvt ) / znvvt
            !gcost(iind,jind) = ( zxnpt*zxvvt + zynpt*zyvvt ) / znvvt

            gsinup = ( zxnpu*zyffu - zynpu*zxffu ) / znffu
            gcosup = ( zxnpu*zxffu + zynpu*zyffu ) / znffu

            !gsinf(iind,jind) = ( zxnpf*zyuuf - zynpf*zxuuf ) / znuuf
            !gcosf(iind,jind) = ( zxnpf*zxuuf + zynpf*zyuuf ) / znuuf

            ! (caution, rotation of 90 degres)
            gsinvp = ( zxnpv*zxffv + zynpv*zyffv ) / znffv
            gcosvp =-( zxnpv*zyffv - zynpv*zxffv ) / znffv
      ...
      !DO jj = 2, jpjglo
        ! DO ji = fs_2, jpiglo   ! vector opt.
            !IF( MOD( ABS( glamv(iind,jind) - glamv(ji,jj-1) ), 360. ) < 1.e-8 ) THEN
            !   gsint(iind,jind) = 0.
            !   gcost(iind,jind) = 1.
            !ENDIF
            IF( MOD( ABS( glamf(iind,jind) - glamf(iind,jind-1) ), 360. ) < 1.e-8 ) THEN
               gsinup = 0.
               gcosup = 1.
            ENDIF
            IF(      ABS( gphif(iind,jind) - gphif(iind-1,jind) )         < 1.e-8 ) THEN
               gsinvp = 0.
               gcosvp = 1.
            ENDIF
            !IF( MOD( ABS( glamu(iind,jind) - glamu(ji,jj+1) ), 360. ) < 1.e-8 ) THEN
            !   gsinf(iind,jind) = 0.
            !   gcosf(iind,jind) = 1.
            !ENDIF
         !END DO
...
!(numout,*) "ANGLE-TEST-preFIN"
	 ...
	 ! WRITE(numout,*) "ANGLE-TEST-FIN"
   END SUBROUTINE angle_point
   ...
   SUBROUTINE rot_rep ( pxin, pyin, cd_type, cdtodo, prot )
      ...
      IF( lmust_init ) THEN
         IF(lwp) WRITE(numout,*)
         IF(lwp) WRITE(numout,*) ' rot_rep : geographic <--> stretche2d'
         IF(lwp) WRITE(numout,*) ' ~~~~~    coordinate transformation'
         !
	  ...
	  !WRITE(numout,*) 'ROT REP DEBUG-OUT', prot
	  
   END SUBROUTINE rot_rep


   SUBROUTINE angle_nompi
      !!----------------------------------------------------------------------
      !!                  ***  ROUTINE angle  ***
      !! 
      !! ** Purpose :   Compute angles between model grid lines and the North direction
...
!CDIR NOVERRCHK
      ...
      ! Lateral boundary conditions !
      ! =========================== !
WRITE(numout,*) "ANGLE-TEST-preFIN"
      ! lateral boundary cond.: T-, U-, V-, F-pts, sgn
      CALL lbc_lnk( gcost, 'T', -1. )   ;   CALL lbc_lnk( gsint, 'T', -1. )
      CALL lbc_lnk( gcosu, 'U', -1. )   ;   CALL lbc_lnk( gsinu, 'U', -1. )
      CALL lbc_lnk( gcosv, 'V', -1. )   ;   CALL lbc_lnk( gsinv, 'V', -1. )
