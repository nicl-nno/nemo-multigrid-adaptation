...
CONTAINS
   ...
   SUBROUTINE bdy_ice_lim( kt )
      ...
      END DO
		DO ib_bdy=1, nb_bdy
		SELECT CASE( cn_ice_dyn2d(ib_bdy) )
		 CASE('none')
            CYCLE
         CASE('frs')
           			CALL bdy_ice_dyn2d_frs( idx_bdy(ib_bdy), dta_bdy(ib_bdy), ib_bdy )
		 CASE DEFAULT
            CALL ctl_stop( 'bdy_ice_lim : unrecognised option for ice drift' )
         END SELECT			
		END DO
#if defined key_lim3
   ...
   END SUBROUTINE bdy_ice_frs
    ...
    SUBROUTINE bdy_ice_dyn2d_frs( idx, dta, ib_bdy)
      !!----------------------------------------------------------------------
...
!		 PRINT *, dta%u_ice(jb)
         u_ice(ii,ij) = ( u_ice(ii,ij) + zwgt * ( dta%u_ice(jb) - u_ice(ii,ij) ) ) * umask(ii,ij,1)
      END DO
      !
      ...
      DO jb = 1, idx%nblen(igrd)
         ...
         ij   = idx%nbj(jb,igrd)
         zwgt = idx%nbw(jb,igrd)
         v_ice(ii,ij) = ( v_ice(ii,ij) + zwgt * ( dta%v_ice(jb) - v_ice(ii,ij) ) ) * vmask(ii,ij,1)
      END DO 
      CALL lbc_bdy_lnk( u_ice(:,:), 'U', -1., ib_bdy )
      CALL lbc_bdy_lnk( v_ice(:,:), 'V', -1., ib_bdy )  ! Boundary points should be updated
      !
!	  PRINT *, 'exit'
      IF( nn_timing == 1 ) CALL timing_stop('bdy_dyn2d_frs')
      !

   END SUBROUTINE bdy_ice_dyn2d_frs  
