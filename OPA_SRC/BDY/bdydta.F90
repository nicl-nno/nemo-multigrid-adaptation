...
!$AGRIF_END_DO_NOT_TREAT
   ...
   INTEGER :: jfld_hti, jfld_hts, jfld_ai,jfld_ui,jfld_vi ! indices of ice thickness, snow thickness and concentration in bf structure
#endif

#  include "domzgr_substitute.h90"
   !!----------------------------------------------------------------------
   !! NEMO/OPA 3.3 , NEMO Consortium (2010)
   !! $Id: bdydta.F90 7859 2017-03-31 13:44:41Z clem $ 
...
CONTAINS
      ...
      REAL(wp) ::   prot_u, prot_v ! temporary variables for vectors rotation
      !!
      !!---------------------------------------------------------------------------
      !!
      ...
      IF( kt .eq. nit000 .and. .not. PRESENT(jit) ) THEN
         ...
         DO ib_bdy = 1, nb_bdy
            ...
            IF( nn_ice_lim_dta(ib_bdy) .eq. 0 ) THEN 
			   ...
			   IF( dta%ll_uvice ) THEN
                  igrd = 2   
                  DO jl = 1, jpl
                     DO ib = 1, ilen1(igrd)
                        ii = idx_bdy(ib_bdy)%nbi(ib,igrd)
                        ij = idx_bdy(ib_bdy)%nbj(ib,igrd)
                        dta_bdy(ib_bdy)%u_ice (ib) =  u_ice(ii,ij) * tmask(ii,ij,1) 
                     END DO
                  END DO
               ENDIF
			   ...
			   IF( dta%ll_uvice ) THEN
                  ...
                  DO jl = 1, jpl
                     DO ib = 1, ilen1(igrd)
                        ii = idx_bdy(ib_bdy)%nbi(ib,igrd)
                        ij = idx_bdy(ib_bdy)%nbj(ib,igrd)
                        dta_bdy(ib_bdy)%v_ice (ib) =  v_ice(ii,ij) * tmask(ii,ij,1) 
                     END DO
                  END DO
               ENDIF
            ENDIF
      ...
      !--------------------------------
     IF(lwp) WRITE(numout,*) 'start update external data from files'
      jstart = 1
      DO ib_bdy = 1, nb_bdy   
         dta => dta_bdy(ib_bdy)
         IF( nn_dta(ib_bdy) .eq. 1 ) THEN ! skip this bit if no external data required
            ...
            ELSE
               ...
               ELSE
                  ...
                  IF( dta%nread(1) .gt. 0 ) THEN ! update external data
                     jend = jstart + dta%nread(1) - 1
					  IF(lwp) WRITE(numout,*) 'before fld read3',jstart,jend,kt,time_offset
                     CALL fld_read( kt=kt, kn_fsbc=1, sd=bf(jstart:jend), &
                                  & map=nbmap_ptr(jstart:jend), kt_offset=time_offset )
					IF(lwp) WRITE(numout,*) 'after fld read3'
                  ENDIF
               ...
               IF( .NOT. ll_bdylim3 .AND. cn_ice_lim(ib_bdy) /= 'none' .AND. nn_ice_lim_dta(ib_bdy) == 1 ) THEN ! bdy ice input (case input is lim2 type)
				...
				IF(lwp) WRITE(numout,*) 'start limvar separation'
				...
				!dta_bdy(ib_bdy)%u_ice(:) = 0.0
				!dta_bdy(ib_bdy)%v_ice(:) = 0.0
				IF(lwp) WRITE(numout,*) 'end limvar separation'
               ENDIF
#endif
            ENDIF
      ...
      !rotation of 2d velocities
      DO ib_bdy = 1, nb_bdy
             IF (cn_tra(ib_bdy) /= 'runoff' .AND. ( nn_dyn2d_dta(ib_bdy) .EQ. 1 .OR. nn_dyn2d_dta(ib_bdy) .EQ. 3 ) )THEN
                igrd = 2                    
			    !WRITE(*,*) "WE ARE HERE"
                DO ib = 1, idx_bdy(ib_bdy)%nblen(igrd)
					 ii   = idx_bdy(ib_bdy)%nbi(ib,igrd)
					 ...
					 CALL rot_rep_isolated(dta_bdy(ib_bdy)%u2d(ib),dta_bdy(ib_bdy)%v2d(ib),ii,ij, 'U', 'en->i', prot_u)
					 CALL rot_rep_isolated(dta_bdy(ib_bdy)%u2d(ib),dta_bdy(ib_bdy)%v2d(ib),ii,ij, 'V', 'en->j', prot_v)
								
					 dta_bdy(ib_bdy)%u2d(ib) = prot_u
					 dta_bdy(ib_bdy)%v2d(ib) = prot_v
                ENDDO
             ENDIF
      ENDDO
		 
      !rotation of 3d velocities
	    DO ib_bdy = 1, nb_bdy
        IF (cn_tra(ib_bdy) /= 'runoff' .AND. ( nn_dyn3d_dta(ib_bdy) .EQ. 1) )THEN
            igrd = 2                    
		    !WRITE(*,*) "WE ARE HERE"
            DO ib = 1, idx_bdy(ib_bdy)%nblen(igrd)
			    DO ik = 1, jpkm1
				    ...
				    CALL rot_rep_isolated(dta_bdy(ib_bdy)%u3d(ib,ik),dta_bdy(ib_bdy)%v3d(ib,ik),ii,ij, 'U', 'en->i', prot_u)
				    CALL rot_rep_isolated(dta_bdy(ib_bdy)%u3d(ib,ik),dta_bdy(ib_bdy)%v3d(ib,ik),ii,ij, 'V', 'en->j', prot_v)
									
				    dta_bdy(ib_bdy)%u3d(ib,ik) = prot_u
				    dta_bdy(ib_bdy)%v3d(ib,ik) = prot_v
			    ENDDO
            ENDDO
        ENDIF
        ENDDO
		...
		!rotation of ice drift
      DO ib_bdy = 1, nb_bdy
             IF (cn_tra(ib_bdy) /= 'runoff' .AND. ( nn_ice_lim_dta(ib_bdy) .eq. 1 )) THEN
				IF(lwp) WRITE(numout,*) 'start ice drift rotation'
                igrd = 2                    
			    !WRITE(*,*) "WE ARE HERE"
                DO ib = 1, idx_bdy(ib_bdy)%nblen(igrd)
					 ii   = idx_bdy(ib_bdy)%nbi(ib,igrd)
					 ...
					 CALL rot_rep_isolated(dta_bdy(ib_bdy)%u_ice(ib),dta_bdy(ib_bdy)%v_ice(ib),ii,ij, 'U', 'en->i', prot_u)
					 CALL rot_rep_isolated(dta_bdy(ib_bdy)%u_ice(ib),dta_bdy(ib_bdy)%v_ice(ib),ii,ij, 'V', 'en->j', prot_v)
								
					 dta_bdy(ib_bdy)%u_ice(ib) = prot_u
					 dta_bdy(ib_bdy)%v_ice(ib) = prot_v
                ENDDO
				IF(lwp) WRITE(numout,*) 'end ice drift rotation'
             ENDIF
      ENDDO
      ...
      DO ib_bdy = 1, nb_bdy         
         ...
         IF( cn_ice_lim(ib_bdy) /= 'none' .and. nn_ice_lim_dta(ib_bdy) .eq. 1  ) THEN
            nb_bdy_fld(ib_bdy) = nb_bdy_fld(ib_bdy) + 5
         ENDIF
#endif               
...
902         IF( ios /= 0 ) CALL ctl_nam ( ios , 'nambdy_dta in configuration namelist', lwp )
           ...
           &  ( ln_full_vel_array(ib_bdy) .and. ( nn_dyn2d_dta(ib_bdy) .eq. 1 .or. nn_dyn2d_dta(ib_bdy) .eq. 3 ) ) ) THEN
            ...
            IF( nn_ice_lim_dta(ib_bdy) .eq. 1 ) THEN
               ...
               IF( dta%ll_a_i ) THEN
			   if(lwp) write(numout,*) '++++++ reading in ai field'
                  jfld = jfld + 1
                  blf_i(jfld) = bn_a_i
                  ibdy(jfld) = ib_bdy
                  igrid(jfld) = 1
               ...
               IF( dta%ll_ht_i ) THEN
			   if(lwp) write(numout,*) '++++++ reading in hti field'
                  jfld = jfld + 1
                  blf_i(jfld) = bn_ht_i
                  ibdy(jfld) = ib_bdy
                  igrid(jfld) = 1
               ...
               IF( dta%ll_ht_s ) THEN
			   if(lwp) write(numout,*) '++++++ reading in hts field'
                  jfld = jfld + 1
                   blf_i(jfld) = bn_ht_s
                  ibdy(jfld) = ib_bdy
                  igrid(jfld) = 1
			   ...
			   IF( dta%ll_uvice ) THEN
                 if(lwp) write(numout,*) '++++++ reading in uice field'
                  jfld = jfld + 1
                  blf_i(jfld) = bn_uice
                  ibdy(jfld) = ib_bdy
                  igrid(jfld) = 1
				  ...
				  if(lwp) write(numout,*) '++++++ reading in vice field'
                  jfld = jfld + 1
                  blf_i(jfld) = bn_vice
                  ibdy(jfld) = ib_bdy
                  igrid(jfld) = 1
            ...
            ELSE
               IF ( ll_bdylim3 ) THEN ! case input is lim3 type
                  ...
                  dta_bdy(ib_bdy)%ht_s => bf(jfld)%fnow(:,1,:)
				   jfld = jfld + 1
                  dta_bdy(ib_bdy)%u_ice => bf(jfld)%fnow(:,1,1)
				   jfld = jfld + 1
                  dta_bdy(ib_bdy)%v_ice => bf(jfld)%fnow(:,1,1)
