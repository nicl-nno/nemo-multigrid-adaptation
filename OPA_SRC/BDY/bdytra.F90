MODULE bdytra
   ...
   USE phycst          ! physical constants (ADDED 30.06.2017 AH)
...
CONTAINS
   ...
   SUBROUTINE bdy_tra( kt )
      ...
      !!
      INTEGER               :: ib_bdy,jj, ji,jk ! Loop index
	  ...
	  IF (( ln_rnf ) .AND. (ln_rnf_forced)) THEN         ! input of heat and salt due to river runoff 
         DO jj = 2, jpj 
            DO ji = 2, jpi
				DO jk = 2, jpkm1
				   IF( rnf(ji,jj) /= 0._wp .AND. tmask(ji,jj,jk) == 1 ) THEN
						IF (sf_t_rnf(1)%fnow(ji,jj,1) > -1.9) THEN
							tsa(ji,jj,jk,jp_tem) = sf_t_rnf(1)%fnow(ji,jj,1)
							!tsa(ji,jj,jk,jp_tem) = -1.0
						ENDIF
						IF (sf_s_rnf(1)%fnow(ji,jj,1) > 0) THEN
							tsa(ji,jj,jk,jp_sal) = sf_s_rnf(1)%fnow(ji,jj,1)
						ENDIF
				   ENDIF
			   END DO    
            END DO    
   ...
   SUBROUTINE bdy_tra_frs( idx, dta, kt )
	  ...
	  REAL(wp) ::   awgt,awgtnum,awgtden           ! additional difference weight
...
!			awgtnum=ABS(tsa(ii,ij,ik,jp_tem)-dta%tem(ib,ik))
!			awgtden=ABS(tsa(ii,ij,ik,jp_tem))+ABS(dta%tem(ib,ik))
!			awgt=1.0-awgtnum/awgtden
!			IF (awgt /= awgt) THEN
!			awgt=0.0
!			ENDIF
!			awgt=zwgt
!			IF (dta%tem(ib,ik) > tsa(ii,ij,ik,jp_tem)) THEN
!			awgt=1.0
!			ENDIF
!			PRINT *, 'ib= ' ,ib, ' T_i=',tsa(ii,ij,ik,jp_tem),' T_e=',dta%tem(ib,ik),' alpha=', zwgt
            tsa(ii,ij,ik,jp_tem) = ( tsa(ii,ij,ik,jp_tem) + zwgt * ( dta%tem(ib,ik) - tsa(ii,ij,ik,jp_tem) ) ) * tmask(ii,ij,ik)   
!            tsa(ii,ij,ik,jp_tem) = ( tsa(ii,ij,ik,jp_tem) + zwgt * ABS(dta%tem(ib,ik)-tsa(ii,ij,ik,jp_tem)))*tmask(ii,ij,ik)  			
!			PRINT *, 'Updated=',tsa(ii,ij,ik,jp_tem)
