...
CONTAINS
   ...
   SUBROUTINE bdy_init
      ...
      NAMELIST/nambdy/ nb_bdy, ln_coords_file, cn_coords_file,                 &
         ...
         &             cn_ice_lim, cn_ice_dyn2d, nn_ice_lim_dta,                           &
      ...
      DO ib_bdy = 1,nb_bdy
        ...
        SELECT CASE( cn_tra(ib_bdy) )                  
		  ...
		  CASE('adv') !(ADDED AH 03.07.2017)
             IF(lwp) WRITE(numout,*) ' ADVection scheme'
             dta_bdy(ib_bdy)%ll_tem = .true.
             dta_bdy(ib_bdy)%ll_sal = .true. 
...
!               idx_bdy(ib_bdy)%nbw(ib,igrd) = 1.- ERF( FLOAT( nbr - 1 ) *0.5 )      ! erf v2 formulation
!               idx_bdy(ib_bdy)%nbw(ib,igrd) = 1.- TANH(FLOAT(nbr - 1)*0.5*FLOAT(nn_rimwidth(ib_bdy)))      ! tanh v3 formulation
!               idx_bdy(ib_bdy)%nbw(ib,igrd) = 1.- ERF(FLOAT(nbr - 1))      ! erf v5 formulation
