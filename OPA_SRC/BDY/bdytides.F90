...
!$AGRIF_END_DO_NOT_TREAT
   ...
   !! $Id: bdytides.F90 5912 2015-11-24 13:42:04Z jchanut $ 
...
902         IF( ios /= 0 ) CALL ctl_nam ( ios , 'nambdy_tide in configuration namelist', lwp )
   ...
   SUBROUTINE bdytide_update ( kt, idx, dta, td, jit, time_offset )
      ...
      !!
      INTEGER, DIMENSION(3)            ::   ilen0       !: length of boundary data (from OBC arrays)
      INTEGER                          :: itide, igrd, ib, ii, ij   ! dummy loop indices
      INTEGER                          :: time_add          ! time offset in units of timesteps
      REAL(wp)                         :: z_arg, z_sarg, zflag, zramp, u_comp, v_comp      
      ...
      DO itide = 1, nb_harmo
         ...
         DO ib = 1, ilen0(igrd)
            ii = idx%nbi(ib,igrd)
            ij = idx%nbj(ib,igrd)
            u_comp = td%u(ib,itide,1)*z_cost(itide) + td%u  (ib,itide,2)*z_sist(itide)
			v_comp = td%v(ib,itide,1)*z_cost(itide) + td%v  (ib,itide,2)*z_sist(itide)
            prot = 0
			CALL rot_rep_isolated(u_comp,v_comp,ii,ij, 'U', 'en->i', prot)
            dta%u2d(ib) = dta%u2d(ib) + zramp * prot
         ...
         DO ib = 1, ilen0(igrd) 
			...
			v_comp = td%v(ib,itide,1)*z_cost(itide) + td%v  (ib,itide,2)*z_sist(itide)
			prot = 0
			CALL rot_rep_isolated(u_comp,v_comp,ii,ij, 'V', 'en->j', prot)
            dta%v2d(ib) = dta%v2d(ib) + zramp*prot      
         END DO
      END DO
      !
   ...
   SUBROUTINE bdy_dta_tides( kt, kit, time_offset )
      ...
      INTEGER  :: itide, ib_bdy, ib, igrd, ii, ij                     ! loop indices
      INTEGER  :: time_add                                    ! time offset in units of timesteps
      REAL(wp) :: z_arg, z_sarg, zramp, zoff, z_cost, z_sist, u_comp, v_comp        
      REAL(wp) ::   prot
      !!----------------------------------------------------------------------
            ...
            DO itide = 1, nb_harmo
               ...
               IF ( dta_bdy(ib_bdy)%ll_u2d ) THEN
                  ...
                  DO ib = 1, ilen0(igrd)
					...
					u_comp = tides(ib_bdy)%u(ib,itide,1)*z_cost + tides(ib_bdy)%u(ib,itide,2)*z_sist
					v_comp = tides(ib_bdy)%v(ib,itide,1)*z_cost + tides(ib_bdy)%v(ib,itide,2)*z_sist
					!WRITE(numout,*) 'FIRST', u_comp,v_comp,ii,ij
					prot = 0
					CALL rot_rep_isolated(u_comp,v_comp,ii,ij, 'U', 'en->i', prot)
                    dta_bdy(ib_bdy)%u2d(ib) = dta_bdy(ib_bdy)%u2d(ib) + prot
                  END DO
               ENDIF
               !
               IF ( dta_bdy(ib_bdy)%ll_v2d ) THEN
                  ...
                  DO ib = 1, ilen0(igrd) 
					 ...
					 prot = 0
					 CALL rot_rep_isolated(u_comp,v_comp,ii,ij, 'V', 'en->j', prot)
                     dta_bdy(ib_bdy)%v2d(ib) = dta_bdy(ib_bdy)%v2d(ib) + prot
                  END DO
               ENDIF
            END DO             
         END IF
